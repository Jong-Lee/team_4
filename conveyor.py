...
class Conveyor(Frame):
    def __init__(self, master, picture, width):
        ...
        self.text = self.conveyor_canvas2.create_text(25, 10, font="Helvetica 11 bold", text="FINAL", fill="blue")
        self.conveyor_canvas2.grid(row=0, column=12)

        # self.conveyor_canvas.grid(row=0, columnspan=13)
        ...
        for i in range(self.n):
            self.labels.append(Label(self, image=self.picture[self.image_number_list[i]], width=40, height=40))
        ...
        # 캔버스 세팅
        # 노란색 역삼각형
        # self.conveyor_canvas.create_rectangle(0, 0, 10, 10, fill='yellow')
        self.conveyor_canvas = Canvas(self, width=30, height=30) # 현재 위치 표시를 위한 캔버스 위젯 생성
        self.triangle = self.conveyor_canvas.create_polygon([(5, 5), (25, 5), (15, 20)], fill='yellow', outline="red")
    ...
    def random_shuffle(self):
        ...
        self.image_number_list = sample(range(0,16), 13)
    ...
    def correct_match(self):
        ...
        # canvas.itemconfig(도형의객체, outline='white', fill='white', + 추가적인 parameter 세팅) 기존에 생성된 도형 객체의 변경 가능
            ...
            if self.cur_idx == self.n-2:
                self.conveyor_canvas2.delete(self.text)
                self.conveyor_canvas.itemconfig(self.triangle, fill='blue', outline='blue')
            else:
                self.conveyor_canvas.itemconfig(self.triangle, fill='yellow', outline='red')
            # 그 외 도형 이동
            # 현재 이미지 및 현재 위치 수정
            self.cur_idx += 1
            self.conveyor_canvas.grid(row=0, column=self.cur_idx)
    ...
    def wrong_match(self):
        ...
        # canvas.itemconfig(도형의객체, outline='white', fill='white', + 추가적인 parameter 세팅) 기존에 생성된 도형 객체의 변경 가능
            ...
            if self.cur_idx == self.n-1:
                self.text = self.conveyor_canvas2.create_text(25, 10, font="Helvetica 11 bold", text="FINAL", fill="blue")
            if self.cur_idx == 1:
                self.conveyor_canvas.itemconfig(self.triangle, fill='red', outline='red')
            else:
                self.conveyor_canvas.itemconfig(self.triangle, fill='yellow', outline='red')
            # 그 외 도형 이동
            self.cur_idx -= 1
